<!DOCTYPE html>
<html lang="en">
  <head>
    <?php require_once('head.php'); ?>
  </head>
  <body>
    <div id="page">
      <?php require_once('header.php'); ?>
      <section class="main mt190">
        <div class="container">
            <div class="template">
              <div class="productdetail_page clearfix">
                <div class="main_content frame clearfix">
                  <ul class="breadcrumb">
                    <li><a href="index.php">Forside</a></li>
                    <li><a href="product.php">GLAS OG PORCELÆN</a></li>
                    <li><a href="product2.php">Stel</a></li>
                    <li><a href="product_detail.php">Stelkompagniet - Tepotte 2,5liter sort stentøj</a></li>
                  </ul>
                  <div class="product_img">
                    <div class="img_larg">
                      <a id="btnLargeImage" class="imgZoom" href="img/thumnail/img_larg.jpg"><img width="430" height="318" src="img/thumnail/img_larg.jpg" alt=""></a>
                    </div>

                    <a id="btnZoomIcon" class="imgZoom btnZoom" href="img/thumnail/img_larg.jpg"><img src="img/icon_zoom.png" alt=""></a>
                    
                    <!--<ul id="thumblist" class="thumail clearfix gallery">
                      <li><a href="#"><img src="img/thumnail/img_larg.jpg" alt=""></a></li>
                      <li><a href="#"><img src="img/thumnail/img_larg.jpg" alt=""></a></li>
                      <li><a href="#"><img src="img/thumnail/img_larg.jpg" alt=""></a></li>
                    </ul>-->                    
                    <hr>
                    <a href="#"><img src="img/icon_face.png" alt=""></a>
                    <div class="clear mb10"></div>
                    <div class="video clearfix">
                      <a class="fl imgZoom" href="https://www.youtube.com/watch?v=-1gQDlgrAQk"><img src="img/thumnail/img_small2.jpg" alt=""></a>
                    </div>
                  </div>
                  <div class="product_content">
                    <h2>Stelkompagniet - Tepotte 2,5liter sort stentøj</h2>
                    <p><strong>Vare-nummer: 30283</strong></p>
                     <div id="scrollbar2">
                      <div class="scrollbar">
                        <div class="track">
                          <div class="thumb">
                            <div class="end"></div>
                          </div>
                        </div>
                      </div>
                      <div class="viewport">
                        <div class="overview">
                          <p>Lorem ipsum dolor sit amet consectetuer adipiscing elit. Donec odio quisque volutpat mattis eros. Nullam malesuada erat ut turpis suspendisse urna nibh viverra non semper suscipit posuere a pede donec nec justo eget felis facilisis fermentum. Aliquam porttitor mauris sit amet orci denean dignissim pellentesque felis.</p>
                          <p>Morbi in sem quis dui placerat ornare. Pellentesque odio nisi, euismod in, pharetra a, ultricies in, diam. Sed arcu. Cras consequat praesent dapibus, neque </p>
                          <p>Lorem ipsum dolor sit amet consectetuer adipiscing elit. Donec odio quisque volutpat mattis eros. Nullam malesuada erat ut turpis suspendisse urna nibh viverra non semper suscipit posuere a pede donec nec justo eget felis facilisis fermentum. Aliquam porttitor mauris sit amet orci denean dignissim pellentesque felis.</p>
                          <p>Morbi in sem quis dui placerat ornare. Pellentesque odio nisi, euismod in, pharetra a, ultricies in, diam. Sed arcu. Cras consequat praesent dapibus, neque </p>
                        </div>
                      </div>
                    </div>

                    <div id="f_size" class="reveal-modal">
                      <div class="f_size clearfix">
                        <h2>STØRRELSESGUIDE</h2>
                        <div class="f_size_content">
                          <div class="size_img">
                            <img src="img/pro01.jpg" alt="">
                          </div>
                          <div class="size_detail">
                            <table>
                              <tr>
                                <td width="40%" class="black">Diameter:</td>
                                <td>20 cm</td>
                              </tr>
                              <tr>
                                <td class="black">Bredde:</td>
                                <td>20 cm</td>
                              </tr>
                              <tr>
                                <td class="black">Længde:</td>
                                <td>20 cm</td>
                              </tr>
                              <tr>
                                <td class="black">Dybde:</td>
                                <td>20 cm</td>
                              </tr>
                              <tr>
                                <td class="black">Højde:</td>
                                <td>20 cm</td>
                              </tr>
                              <tr>
                                <td class="black">Sædehøjde:</td>
                                <td>20 cm</td>
                              </tr>
                            </table>
                          </div>
                        </div>
                      </div>
                       <a id="close-reveal-modal2" class="close-reveal-modal"></a>
                    </div>

                    <a class="btn_sizeguide cblack" href="#" data-reveal-id="f_size">STØRRELSESGUIDE</a>
                    <h3><span class="price_old">Førpris: 529 DKK</span> (De sparer: 50 DKK)</h3>
                    <div class="number">
                      <label for="">Antal:</label>
                      <input type="text" placeholder="1">
                      <h2 class="price">479 DKK</h2>
                    </div>
                    <div class="fr mt10"><img src="img/icon_checkgreen.png" alt=""></div>
                    <div class="clear mb20"></div>
                    <ul class="option clearfix">
                      <li>
                        <input id="c1" type="radio" name="cc" checked>
                        <label for="c1">Lucie Antique Terracotta</label>
                      </li>
                      <li>
                        <input id="c2" type="radio" name="cc">
                        <label for="c2">Beatrice Antique Terracotta</label>
                      </li>
                      <li>
                        <input id="c3" type="radio" name="cc">
                        <label for="c3">Tomasso Antique Terracotta</label>
                      </li>
                    </ul>

                    <ul class="option clearfix bb1 mb20">
                      <li>
                        <input id="c4" type="radio" name="cc2" checked>
                        <label for="c4">Share sidebordunderstel 60 cm</label>
                      </li>
                      <li>
                        <input id="c5" type="radio" name="cc2" checked>
                        <label for="c5">Bordplade 50x60 cm, Hvid matteret hærdet glas</label>
                      </li>
                    </ul>

                    <a id="btnAddcart" class="btnAddcart cblack" href="#">Tilføj indkøbskurven</a>
                  </div>
                </div>
                
                <div class="related_product">
                  <h2><img src="img/title_relateproduct.png" alt=""></h2>
                  <div class="products">
                    <ul class="clearfix">
                      <li>
                        <div class="img_main">
                          <a href="#"><img src="img/img01.jpg" alt=""></a>
                        </div>
                        <h3>Diamond Loungestol Hvid Tex®</h3>
                        <p class="price_before">Førpris: 529 DKK</p>
                        <p class="price_sale">(De sparer: 50 DKK) </p>
                        <h4>479 DKK</h4>
                        <div class="pro-larg animated clearfix">
                          <div class="img_main">
                            <a href="product_detail.php"><img src="img/img01.jpg" alt=""></a>
                          </div>
                          <h3>Diamond Loungestol Hvid Tex®</h3>
                          <p class="no_number">Vare-nummer: 30283</p>
                          <p class="price_before">Førpris: 529 DKK</p>
                          <p class="price_sale">(De sparer: 50 DKK) </p>
                          <h4>479 DKK</h4>
                          <a class="btnMore btn2" href="product_detail.php">Vis detaljer</a>
                        </div>
                      </li>
                      <li>
                        <div class="img_main">
                          <a href="#"><img src="img/img02.jpg" alt=""></a>
                        </div>
                        <h3>Diamond 2-pers. sofa Hvid Tex®</h3>
                        <p class="price_before">Førpris: 529 DKK</p>
                        <p class="price_sale">(De sparer: 50 DKK) </p>
                        <h4>479 DKK</h4>
                        <div class="pro-larg animated clearfix">
                          <div class="img_main">
                            <a href="product_detail.php"><img src="img/img01.jpg" alt=""></a>
                          </div>
                          <h3>Diamond 2-pers. sofa Hvid Tex®</h3>
                          <p class="no_number">Vare-nummer: 30283</p>
                          <p class="price_before">Førpris: 529 DKK</p>
                          <p class="price_sale">(De sparer: 50 DKK) </p>
                          <h4>479 DKK</h4>
                          <a class="btnMore btn2" href="product_detail.php">Vis detaljer</a>
                        </div>
                      </li>
                      <li>
                        <div class="img_main">
                          <a href="#"><img src="img/img01.jpg" alt=""></a>
                        </div>
                        <h3>Diamond 3-pers. sofa Hvid Tex®</h3>
                        <p class="price_before">Førpris: 529 DKK</p>
                        <p class="price_sale">(De sparer: 50 DKK) </p>
                        <h4>479 DKK</h4>
                        <div class="pro-larg animated clearfix">
                          <div class="img_main">
                            <a href="product_detail.php"><img src="img/img01.jpg" alt=""></a>
                          </div>
                          <h3>Diamond 3-pers. sofa Hvid Tex®</h3>
                          <p class="no_number">Vare-nummer: 30283</p>
                          <p class="price_before">Førpris: 529 DKK</p>
                          <p class="price_sale">(De sparer: 50 DKK) </p>
                          <h4>479 DKK</h4>
                          <a class="btnMore btn2" href="product_detail.php">Vis detaljer</a>
                        </div>
                      </li>
                      <li>
                        <div class="img_main">
                          <a href="#"><img src="img/img01.jpg" alt=""></a>
                        </div>
                        <h3>Diamond Loungestol Hvid Tex®</h3>
                        <p class="price_before">Førpris: 529 DKK</p>
                        <p class="price_sale">(De sparer: 50 DKK) </p>
                        <h4>479 DKK</h4>
                        <div class="pro-larg animated clearfix">
                          <div class="img_main">
                            <a href="product_detail.php"><img src="img/img01.jpg" alt=""></a>
                          </div>
                          <h3>Diamond Loungestol Hvid Tex®</h3>
                          <p class="no_number">Vare-nummer: 30283</p>
                          <p class="price_before">Førpris: 529 DKK</p>
                          <p class="price_sale">(De sparer: 50 DKK) </p>
                          <h4>479 DKK</h4>
                          <a class="btnMore btn2" href="product_detail.php">Vis detaljer</a>
                        </div>
                      </li>
                      </ul>
                  </div>
                </div>

              </div>
            </div>
        </div>

      </section>
      <?php require_once('delivery.php'); ?>
      <?php require_once('footer.php'); ?>
    </div>

   <?php require_once('js-footer.php'); ?>
  </body>
</html>